...
const int qParam=8;// Pi/q: angle beween reflexion planes b and c .
const int rParam=3;// Pi/r: angle beween reflexion planes c and a .
...
const float SRadius=0.007;//Thikness of the lines
...
float dist2Segment(vec3 z, vec3 n, float r){
    ...
    //return tha-.05;
    //return DD(tha,z.y)-SRadius;
//
//    return hlengths(p-hnormalizet(z))/10.;
    return DD((tha-SRadius*6.)/(1.+spaceType*tha*SRadius),z.x);
}
...
vec2 cLog(vec2 a) {
    ...
    //if (b>0.0) b-=2.0*3.14159;
    return vec2(log(length(a)),b);
}

uniform float uiA;//0;.1;.3
uniform float uiB;//.7;.1;1.

void main() {
	const float scaleFactor=5.1;
...
//	animUVW(0.5*PI*u_time);
    ...
    float angle = atan(uiA);
    uv = cDiv(uv, cExp(vec2(0,angle))*uiB); 
    // 2. Tile the strips
    uv.x+=u_time;
    uv.x = mod(uv.x,1.6);
    uv.x-=.78;
