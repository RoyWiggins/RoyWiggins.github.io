...
const int qParam=4;// Pi/q: angle beween reflexion planes b and c .
...
void main( )
{
    ...
    aaScale=.4*scaleFactor/u_resolution.y;
	animUVW(.5);//0.5*PI*fract(u_time));
	init(); 
    uv-=vec2(-0.220,-0.250);
	uv = cInverse(uv * 13.);
	gl_FragColor = vec4(test(uv*1.3+vec2(-0.320,0.500)),1.0);
}
