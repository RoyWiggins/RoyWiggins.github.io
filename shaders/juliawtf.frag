...
vec3 M(vec2 z,vec2 c){
    ...
    vec2 CA = vec2(-0.320,-0.390);
    vec2 CB = vec2(0.610,0.580);//cis(u_time)/20.;
    vec2 CC = vec2(0.880,-0.630);
    vec2 CD = vec2(0.290,0.970);
    vec3 color;
    for (int i=0; i<30;i++){
        if( dot(z,z)>1000000.0 ) continue;
        z = cMul(z,z);
        ...
        color = (1.-amount)*color+amount*length(z)*domain(z)*domain2(z);
    }
