...
const int pParam=8;// Pi/p: angle beween reflexion planes a and b .
const int qParam=2;// Pi/q: angle beween reflexion planes b and c .
...
const float SRadius=.04;//Thikness of the lines
...
vec3 color(vec2 pos){
	...
	//antialiasing using distance de segments and vertices (ds and dv) (see:http://www.iquilezles.org/www/articles/distance/distance.htm)
	{
        ...
        color=vec3(clamp(ds/aaScale,0.,1.));
	}
...
vec3 test(vec2 z) {
    float dist=1.;
    z = z * 2.;
    vec2 c = z;//vec2(-.1,sin(fract(u_time/2.)*2.*PI)/1.5);
    const int n = 34;
    vec2 phi = z;
    float k = 0.;
    for (int i=0; i<n;i++){

        if( dot(z,z)>60.0 ) continue;
        k = k+1.;
        // point
        z = cPower(z,2.) + c;
        vec2 a = cDiv(z,z-c);
...
void main( )
{
    ...
    aaScale=.04*scaleFactor/u_resolution.y;
    uv.x-=.25;
	animUVW(.5);//0.5*PI*fract(u_time));
	init(); 
	//uv = cInverse(uv);
	gl_FragColor = vec4(test(uv),1.0);
}
