...
void main()
{
   ...
   float ang, si, co;
   ang = (iMouse.x / iResolution.x*.5)+4.912;
   ...
   mat4 cam_mat = mat4(
     ...
     -si, 0., co, 0.,
      0., 0., 0., 1.);
   ang = //(iMouse.y - iResolution.y*.5)*-.003 ;//* -.003;
      .6; //iGlobalTime*.2;
   si = sin(ang); co = cos(ang);
...
float fField(vec3 p)
   ...
   //float sphere = length(p-vec3(1.+sin(iGlobalTime*.25)*.2,.8,1))-1.;
    ...
    p = wrap(p, lRadius,80.,lRadius+sRadius/2.+25.168,700.);
    if ( n > 2 )
    p = wrap(p, lRadius,20.,lRadius+sRadius/2.+5.,150.);
	if ( n > 3 )
    p = wrap(p, lRadius,4.,lRadius+sRadius/2.+1.,40.);
    if ( n > 4 )
	p = wrap(p, sRadius,1.,lRadius+sRadius/2.,35.);
    return helix(p,sRadius,lRadius*c,0.628);/*    p.y = mod(p.y,25.)
    d = level(p);
    p -= vec3(0 ,25, 0);
    d = min(d, level4(p));
	return d;*/
