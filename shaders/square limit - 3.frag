...
float bird(vec2 z){
    float color = smoothstep(-1.01,-1.,-abs(z.y*-2.-3.));
    color *= smoothstep(-0.00001,0.,-max(z.y-z.x,z.x+z.y))-smoothstep(1.,1.01,-max(z.y-z.x,z.x+z.y+1.));
    color = mix(color,1.,smoothstep(-1.01,-1.,-abs(2.0*z.x-1.))*step(.0,-z.y)*(1.-smoothstep(1.,1.001,-min(z.y-z.x+2.,z.x+z.y+1.))));
    color *= smoothstep(.1,.11,length(z+vec2(1.,1.490)));
    return color;
}
...
vec3 tiles(vec2 z){
    z = z * vec2(2.,1.);
    vec3 color = tile(z*rot_mat2(PI)+vec2(1.,1.),0);
        vec3 tile2 = tile(z*rot_mat2(PI/2.)-vec2(-1,1.),1);
    color = max(vec3(0.0),color - tile2)+tile2;
...
vec3 tiles2(vec2 z){
    z = z * vec2(2.,1.);
    vec3 color = tile(z*rot_mat2(PI)+vec2(1.,1.),3);
    vec3 tile2 = tile(z*rot_mat2(PI/2.)-vec2(-1,1),2);
...
void main( )
{
    ...
    gl_FragColor =vec4(color(uv),1.);//
    //gl_FragColor = vec4(vec3(tiles2(uv*rot_mat2(-PI/2.)*vec2(.5,1)+vec2(0.5,0.20))),1.000);
}
