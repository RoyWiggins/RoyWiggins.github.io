...
const int pParam=4;// Pi/p: angle beween reflexion planes a and b .
...
void main( )
{
	const float scaleFactor=2.6;
	vec2 uv = scaleFactor*(gl_FragCoord.xy-0.5*u_resolution.xy) / u_resolution.y;
    if (uv.x>0. && uv.y < 0.){
        uv *= mat2(cos(PI/2.),sin(PI/2.),-sin(PI/2.),cos(PI/2.));
    }
    if (uv.x<0. && uv.y > 0.){
        uv *= mat2(cos(PI/2.),sin(PI/2.),-sin(PI/2.),cos(PI/2.));
    }
    ...
    //    if (dot(uv,uv)>1.)
//        uv = cInverse(uv);
    aaScale=1.4*scaleFactor/u_resolution.y*length(uv);
